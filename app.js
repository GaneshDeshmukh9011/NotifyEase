...
//Encrypt Data
const encryptData = async (inputData) => {
  return new Promise((resolve, reject) => {
    const pythonProcess = spawn("python", ["./kyber_operations.py","encrypt_message"]);
...
app.post("/api/data/sendMessage", async (req, res) => {
  try {
    ...
    for (const item of data.rollNo) { // ✅ Replaced map() with for...of
      const id = uuidv4();
      ...
      const publicKeyData = await newPublicKey.findOne({ rollno: item.rollNo });
      ...
      try {
        ...
        await message.create({
          studentId: encryptedData.studentId, // ✅ Fixed: Was `encryptedData.rollNo`
          messageId: encryptedData.messageId, // ✅ Fixed: Was `encryptedData.id`
