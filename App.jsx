      ...
      <Router>
        <div className='h-screen w-screen'>
          <header className='w-full border-b flex justify-between h-[10%]'>  
              <div className='w-[70%] py-2 px-14 font-medium text-xl flex items-center text-green-700'>
                <Link>notify<span className='text-blue-500'>Ease</span></Link>
              </div>
              {isLogin === "1" ? (
                <div className='flex w-[30%] justify-around items-center p-2 font-medium'>
                  <Link to="/" onClick={() => { handleanotherClick(1) }} className={`${activeNav === 1 ? "border-b-2" : ""} border-blue-500 rounded-lg p-2`}>
                    <div>Home</div>
                  </Link>
                  {role === "admin" ? (
                    <Link to="/addRecord" onClick={() => { handleanotherClick(2) }} className={`${activeNav === 2 ? "border-b-2" : ""} border-blue-500 rounded-lg p-2`}>
                      <div>Add Record</div>
                    </Link>
                  ) : ""}
                  <Link className='px-3 py-1 bg-blue-600 text-white rounded-md'>
            ...
            <div className={`h-full ${isLogin === "1" && role === "admin" ? "w-[82%]" : "w-[100%]"}`}>
              <Routes>
                {isLogin === "1" ? (
                  ...
                  <Route path="*" element={<Login userLogin={userLogin} setUserRole={setUserRole} setUserLogData={setUserLogData} />} />
                )}
